...
export default function WeeklyLoans() {
  const router = useRouter();
  const [loans, setLoans] = useState<WeeklyLoan[]>([]);
  const [filteredLoans, setFilteredLoans] = useState<WeeklyLoan[]>([]);
  ...
  useEffect(() => {
    const fetchWeeklyLoans = async () => {
      try {
        ...
        setLoans(data.data);
        setFilteredLoans(
          data.data.filter((loan: WeeklyLoan) => loan.status === "active")
        );
      } catch (error: unknown) {
  ...
  const handleCreateNewLoan = () => {
    ...
    router.push("/dashboard/weekly/new");
  };
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-start sm:items-center justify-between">
          ...
          <div className="flex-1 w-full sm:max-w-md">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                id="search"
                className="block w-full pl-8 sm:pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-green-500 focus:border-green-500 text-sm"
          ...
          <div className="flex items-center space-x-2 sm:space-x-4 w-full sm:w-auto">
            ...
            <select
              id="status-filter"
              className="block w-full sm:w-auto pl-3 pr-8 py-2 text-sm border border-gray-300 bg-white text-gray-900 focus:outline-none focus:ring-green-500 focus:border-green-500 rounded-md"
              value={statusFilter}
              onChange={(e) =>
                setStatusFilter(
          ...
          <button
            ...
            className="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white px-3 sm:px-4 py-2 rounded-md text-xs sm:text-sm font-medium transition duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
          >
            Create New Loan
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {filteredLoans.map((loan, index) => (
          <div
            key={loan._id}
            className="bg-white rounded-lg shadow-md hover:shadow-lg transition duration-200 ease-in-out p-4 sm:p-6 border-l-4 border-green-500 cursor-pointer"
