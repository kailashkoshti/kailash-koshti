...
export default function WeeklyLoanDetail() {
  const router = useRouter();
  const params = useParams();
  const loanId = params.id as string;

  const [loan, setLoan] = useState<WeeklyLoan | null>(null);
  ...
  const fetchWeeklyLoan = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/weekly/${loanId}`,
        {
          method: "GET",
          headers: {
  ...
  const handleAddInstallment = () => {
    ...
    setLoan(updatedLoan as WeeklyLoan);
  ...
  const handleInstallmentUpdate = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/weekly/${loanId}/installments`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleMarkAsPaid = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("accessToken");
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/weekly/${loanId}/mark-paid`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
          ...
          {loan.status === "active" &&
            loan.collectedAmount < loan.loanAmount && (
              <button
                ...
                disabled={updating}
                className="px-3 sm:px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 disabled:opacity-50 flex items-center justify-center space-x-2 text-sm sm:text-base"
              >
                <svg
                  className="h-4 w-4"
                  fill="none"
